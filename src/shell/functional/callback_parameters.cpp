...
namespace linuxplorer::shell::functional {
	...
	{		
		this->m_absolute_placeholder_path = std::filesystem::path(info->VolumeDosName) / std::filesystem::path(info->NormalizedPath);
	}
	...
	const std::filesystem::path& callback_parameters::get_absolute_placeholder_path() const noexcept{
	...
	namespace specialized {
		...
		{
			if (parameters->Rename.TargetPath) this->m_absolute_new_path = std::filesystem::path(info->VolumeDosName) / std::filesystem::path(parameters->Rename.TargetPath);
		}

		const std::filesystem::path& rename_callback_parameters::get_absolute_new_path() const noexcept {
		...
		rename_completion_callback_parameters::rename_completion_callback_parameters(const ::CF_CALLBACK_INFO* info, const ::CF_CALLBACK_PARAMETERS* parameters) : callback_parameters(info, parameters)
		{
			if (parameters->RenameCompletion.SourcePath) {
				this->m_absolute_old_path = std::filesystem::path(info->VolumeDosName) / std::filesystem::path(parameters->RenameCompletion.SourcePath);
			}
		}

		const std::filesystem::path& rename_completion_callback_parameters::get_absolute_old_path() const noexcept {
