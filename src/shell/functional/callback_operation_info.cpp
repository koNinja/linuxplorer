...
namespace linuxplorer::shell::functional::specialized {
	...
	std::size_t fetch_data_operation_info_yielded::get_length() const noexcept {
		return this->m_length;
	}
	void fetch_data_operation_info_yielded::set_length(std::size_t length) noexcept {
		this->m_length = length;
	}

	std::span<const std::byte> fetch_data_operation_info_yielded::get_buffer() const noexcept {
		return std::span<const std::byte>(this->m_buffer);
	}
	void fetch_data_operation_info_yielded::set_buffer(const std::vector<std::byte>& buffer) noexcept {
		this->m_buffer = buffer;
	}

	void fetch_data_operation_info_yielded::set_buffer(std::vector<std::byte>&& buffer) noexcept {
	...
	void fetch_placeholders_operation_info::add_creation_info(filesystem::placeholder_creation_info&& info) {
		this->m_creation_info.push_back(std::move(info));
	}
