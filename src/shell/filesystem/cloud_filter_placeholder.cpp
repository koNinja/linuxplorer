...
namespace linuxplorer::shell::filesystem {
	cloud_filter_placeholder::cloud_filter_placeholder(const cloud_provider_session& session, const std::filesystem::path& relative_path) {
		this->m_absolute_path = session.get_sync_root_dir() / relative_path;
		this->open_handle();
		this->fetch();
	}

	cloud_filter_placeholder::cloud_filter_placeholder(const std::filesystem::path& absolute_path) : m_absolute_path(absolute_path) {		
		this->open_handle();
		this->fetch();
	}
	...
	cloud_filter_placeholder cloud_filter_placeholder::create(const cloud_provider_session& session, const placeholder_creation_info& metadata) {		
		const auto& relative_path = metadata.get_relative_path();
		auto filename = relative_path.filename();
		auto dir_path = relative_path.parent_path();
		...
		create_info.RelativeFileName = filename.c_str();
		create_info.FsMetadata = nt_metadata;
		create_info.FileIdentity = metadata.get_identity().data();
		create_info.FileIdentityLength = metadata.get_identity().size_bytes();
		create_info.Flags = ::CF_PLACEHOLDER_CREATE_FLAGS::CF_PLACEHOLDER_CREATE_FLAG_MARK_IN_SYNC;		

		auto absolute_dir_path = session.get_sync_root_dir() / dir_path;
	...
	cloud_filter_placeholder cloud_filter_placeholder::create(const std::filesystem::path& syncroot_path, const placeholder_creation_info& metadata) {		
		const auto& relative_path = metadata.get_relative_path();
		auto filename = relative_path.filename();
		auto dir_path = relative_path.parent_path();
		...
		auto absolute_dir_path = syncroot_path / dir_path;
		...
		return cloud_filter_placeholder(syncroot_path / relative_path);
	}

	cloud_filter_placeholder cloud_filter_placeholder::transform(const cloud_provider_session& session, const std::filesystem::path& relative_path, std::span<const std::byte> identity) {
		using nt_unique_handle = std::unique_ptr<std::remove_pointer_t<::HANDLE>, decltype([](::HANDLE handle) { ::CloseHandle(handle); })>;
		std::filesystem::path absolute_path = session.get_sync_root_dir() / relative_path;
	...
	cloud_filter_placeholder cloud_filter_placeholder::transform(const std::filesystem::path& absolute_path, std::span<const std::byte> identity) {
		using nt_unique_handle = std::unique_ptr<std::remove_pointer_t<::HANDLE>, decltype([](::HANDLE handle) { ::CloseHandle(handle); })>;
		...
		return cloud_filter_placeholder(absolute_path);
	}
	...
	void cloud_filter_placeholder::revert(cloud_filter_placeholder&& placeholder) {
		::HANDLE handle = placeholder.m_handle;
		placeholder.m_handle = INVALID_HANDLE_VALUE;
	...
	bool cloud_filter_placeholder::is_placeholder(const cloud_provider_session& session, const std::filesystem::path& relative_path) {
		std::filesystem::path absolute_path = session.get_sync_root_dir() / relative_path;
	...
	bool cloud_filter_placeholder::is_placeholder(const std::filesystem::path& absolute_path) {
		using unique_nthandle = std::unique_ptr<std::remove_pointer_t<::HANDLE>, decltype([](::HANDLE hd) { ::CloseHandle(hd); })>;
		unique_nthandle handle(::CreateFileW(
			absolute_path.c_str(),
			READ_ATTRIBUTES,
	...
	void cloud_filter_placeholder::internal_primary_fetch() {
		...
		this->m_pin_state = static_cast<placeholder_pin_state>(placeholder_info->PinState);
	...
	void cloud_filter_placeholder::internal_primary_flush() const {
		...
		hr = ::CfSetPinState(this->m_handle, static_cast<::CF_PIN_STATE>(this->m_pin_state), ::CF_SET_PIN_FLAGS::CF_SET_PIN_FLAG_RECURSE, nullptr);
		if (FAILED(hr)) {
			std::error_code ec(hr, std::system_category());
	...
	const std::filesystem::path& cloud_filter_placeholder::get_path() const noexcept {
	...
	placeholder_pin_state cloud_filter_placeholder::get_pin_state() const noexcept {
		return this->m_pin_state;
	}
	void cloud_filter_placeholder::set_pin_state(placeholder_pin_state state) noexcept {
		this->m_pin_state = state;
	}

	std::span<const std::byte> cloud_filter_placeholder::get_identity() const noexcept {
	...
	file_placeholder::file_placeholder(const cloud_provider_session& session, const std::filesystem::path& relative_path) : cloud_filter_placeholder(session, relative_path) {
		this->internal_secondary_fetch();
	}

	file_placeholder::file_placeholder(const std::filesystem::path& absolute_path) : cloud_filter_placeholder(absolute_path) {
		this->internal_secondary_fetch();
	}
	...
	void file_placeholder::dehydrate_unsafe() {
		this->internal_dehydrate_unsafe(0, std::nullopt);
	}

	void file_placeholder::dehydrate_unsafe(std::size_t offset, std::size_t length) {
		this->internal_dehydrate_unsafe(offset, length);
	}

	void file_placeholder::internal_dehydrate_unsafe(std::size_t offset, std::optional<std::size_t> length) {
		if (this->get_type() != placeholder_type::file) {
			throw cloud_provider_runtime_exception("Hydration not supported for non-file type placeholders.");
		}

		::CF_FILE_RANGE range;
		range.Length.QuadPart = length.value_or(CF_EOF);
		range.StartingOffset.QuadPart = offset;

		::HRESULT hr = ::CfUpdatePlaceholder(this->get_handle(), nullptr, nullptr, 0, &range, 1, ::CF_UPDATE_FLAGS::CF_UPDATE_FLAG_NONE, nullptr, nullptr);
		if (FAILED(hr)) {
			std::error_code ec(hr, std::system_category());
	...
	directory_placeholder::directory_placeholder(const cloud_provider_session& session, const std::filesystem::path& relative_path) : cloud_filter_placeholder(session, relative_path) {
		this->internal_secondary_fetch();
	}

	directory_placeholder::directory_placeholder(const std::filesystem::path& absolute_path) : cloud_filter_placeholder(absolute_path) {
		this->internal_secondary_fetch();
	}
