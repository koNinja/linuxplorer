...
namespace linuxplorer::util::charset {
	std::wstring multibyte_wide_compat_helper::convert_multibyte_to_wide(std::string_view source) {
		...
		int dst_len = ::MultiByteToWideChar(CP_UTF8, 0, source.data(), src_len, nullptr, 0);

		auto str = std::make_unique<wchar_t[]>(dst_len + 1);

		auto ec = ::MultiByteToWideChar(CP_UTF8, 0, source.data(), src_len * sizeof(char), str.get(), dst_len + 1);
		if (ec <= 0) {
	...
	std::string multibyte_wide_compat_helper::convert_wide_to_multibyte(std::wstring_view source) {
		...
		int dst_len = ::WideCharToMultiByte(CP_UTF8, 0, source.data(), src_len, nullptr, 0, nullptr, nullptr);
		
		auto str = std::make_unique<char[]>(dst_len + 1);

		auto ec = ::WideCharToMultiByte(CP_UTF8, 0, source.data(), src_len, str.get(), (dst_len + 1) * sizeof(char), nullptr, nullptr);
		if (ec <= 0) {
			std::error_code ec(::GetLastError(), std::system_category());
			throw std::system_error(ec, "Failed to convert wide type to multibyte type.");
		}
