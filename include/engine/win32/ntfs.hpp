#ifndef LINUXPLORER_ENGINE_NTFS_HPP_
#define LINUXPLORER_ENGINE_NTFS_HPP_
...
namespace linuxplorer::engine::win32 {
	struct LINUXPLORER_ENGINE_API file_reference_number {
	private:
		::FILE_ID_128 m_frn;
	public:
	...
	file_reference_number LINUXPLORER_ENGINE_API get_frn(const std::filesystem::path& path);
}

namespace std {
	template <>
	struct hash<linuxplorer::engine::win32::file_reference_number> {
		size_t operator()(const linuxplorer::engine::win32::file_reference_number& key) const noexcept {
...
#endif // LINUXPLORER_ENGINE_NTFS_HPP_